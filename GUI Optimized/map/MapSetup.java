...
public class MapSetup { 
	...
	public static List<Position> createEnemyPositions(int count, int size, boolean bool){ //Put in diff methods
...
//  - - - - - Check for valid enemy move; if move is valid, grid is updated
	public static boolean checkEnemyMove(GridPane grid, Position newPos, Enemy e){
		...
		return !(newPosition != null && (newPosition.getFill().equals(terrainImg) || newPosition.getFill().equals(playerImg) || newPosition.getFill().equals(wallImg)
...
//		- - - - - Updates map per valid move
	public static void updateGrid(GridPane grid, Position newPosition, Player player, Stage window) {
		...
		if (melee || ranged) {
			...
			while(player.getStats().getHealth() > 0 && enemy != null && enemy.getStats().getHealth() > 0) {
				BattleThread battle = new BattleThread(); // Make new method and call
				battle.start(grid, player, melee, ranged, newPosition, cell, window);
			}
		}

		if (cell != null && (cell.getFill().equals(def_hover) || cell.getFill().equals(str_hover) || cell.getFill().equals(mag_hover))) {
			Main.pickUpItemWindow(grid, pot, newPosition, cell, player); //Rename to openPickupItemWindow
		}
	}
...
//	- - - - - - - - Enemy drops/ looting items
	public static void enemyDrop(GridPane grid, Unit unit) {
		Rectangle oldC = getNode(grid, unit.getPosition());
		if (oldC == null) {
			return;
		}

		potionType = new Random().nextInt(30);
		Inventory pot = new Inventory(potionType);
...
//	- - - - - - - - Checking enemies around player
	public static boolean checkRanged(GridPane grid, Position p, Player player) { // Returns true if there is enemy diagonal
		...
		return newPosition != null && newPosition.getFill().equals(enemy_hover);
	}
	...
	public static ImagePattern enterHover(Rectangle cell) { // Put into 1 method and change into hashMap
