...
public class MapSetup { 
	...
	public static final ImagePattern bossImg = new ImagePattern(new Image("/entities/bossFull.png"));
...
//	- - - - - Checks for valid move; if move is valid, grid is updated
	public static boolean checkMove(GridPane grid, Position p, Player player) {
		...
		return newPosition.getFill().equals(floor_hover) || ( (numberOfEnemies(grid) == 0 || !isBossAlive(grid) )&& newPosition.getFill().equals(portalImg)); //If location empty, moves, if no more enemies, portal to next level opens
	}
...
//  - - - - - Check for valid enemy move; if move is valid, grid is updated
	public static boolean checkEnemyMove(GridPane grid, Position newPos){
		...
		boolean Move = !(newPosition.getFill().equals(terrainImg) || newPosition.getFill().equals(playerImg) || newPosition.getFill().equals(wallImg) 
				         ...
				         || newPosition.getFill().equals(enemyImg)|| newPosition.getFill().equals(portalImg));
...
//	- - - - - - - - Checking enemies around player
	...
	public static int numberOfEnemies(GridPane grid) { // Iterates through each cell
		...
		for (Node node : grid.getChildren()) {
			...
			boolean enemyExists = cell.getFill().equals(enemyImg) || cell.getFill().equals(enemy_hover);
			if (enemyExists) {
				enemies++;
			}
		}
	...
	public static boolean isBossAlive(GridPane grid) { // Iterates through each cell
		...
		for (Node node : grid.getChildren()) {
			Rectangle cell = (Rectangle)node;
			boolean bossExists = cell.getFill().equals(bossImg) || cell.getFill().equals(boss_hover);
			if (bossExists) {
				b++;
			}
		}
	...
	public static ImagePattern enterHover(GridPane grid, Position p, Rectangle cell) {
		...
		if (cell.getFill().equals(BattleThread.boss3)) {return BattleThread.boss3Hover;}
		if (cell.getFill().equals(BattleThread.boss2)) {return BattleThread.boss2Hover;}
		if (cell.getFill().equals(BattleThread.boss1)) {return BattleThread.boss1Hover;}
		if (cell.getFill().equals(bossImg)) {return boss_hover;}
		return wallImg;
	}
	...
	public static ImagePattern exitHover(GridPane grid, Position p, Rectangle cell) {
		...
		if (cell.getFill().equals(boss_hover)) {return bossImg;}
		...
		if (cell.getFill().equals(BattleThread.boss3Hover)) {return BattleThread.boss3;}
		if (cell.getFill().equals(BattleThread.boss2Hover)) {return BattleThread.boss2;}
		if (cell.getFill().equals(BattleThread.boss1Hover)) {return BattleThread.boss1;}
		return wallImg;
	}
}