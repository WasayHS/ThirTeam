...
public class GameState {
	...
	public void victory(Stage finished) throws Exception{
		...
		/// series of text to end the story (using the "Label" object).....
	...
	public void openPortal(GridPane grid, Position p, Player player) { // Open portal to next round; event handling: send player to new map.
		if (MapSetup.checkMove(grid, p, player)) {
			TextField tf = MapSetup.getNode(grid, p);
			tf.setText(MapSetup.PLAYER);
			tf.setStyle("-fx-text-inner-color: lightgreen;-fx-font-weight: bold;"); 
			
			TextField oldTf = MapSetup.getNode(grid, p);
